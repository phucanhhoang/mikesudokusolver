...
public class ParallelStrategySolver {
	 ...
	 */
	public void setupPossibleValues(CountingSemaphore counter, ThreadPoolExecutor threadPool) {
		...
		//System.out.println("Should equal number of squares filled :" + counter);
	...
	private void loopThroughStrategies(CountingSemaphore counter, ThreadPoolExecutor threadPool) {
		// ----------------------------
	...
	private void startRowAndColumnAndSquareThreads(CountingSemaphore counter, ThreadPoolExecutor threadPool) {
	...
	private void startSquareThread(CountingSemaphore counter, int square, ThreadPoolExecutor threadPool) {
		startThread( new OnePossibilitySquareStrategy(counter, square, board, possibleValues), threadPool);
	}

	private void startRowThread(CountingSemaphore counter, int row, ThreadPoolExecutor threadPool) {
		startThread( new OnePossibilityStrategy(counter, row, board, possibleValues, true), threadPool);
	}
	
	private void startColThread(CountingSemaphore counter, int col, ThreadPoolExecutor threadPool) {
		startThread( new OnePossibilityStrategy(counter, col, board, possibleValues, false),threadPool);
	}

	private void startSingleValueForCellThreads(CountingSemaphore counter, ThreadPoolExecutor threadPool) {
		// Create the thread pool.
		for (int m = 0; m < Constants.BOARD_SIZE; m++) {
			for (int n = 0; n < Constants.BOARD_SIZE; n++) {
				if (board.getValueAt(m,n) == Constants.EMPTY_CELL) {
					try {
						//new Thread(new SingleValueForCellStrategy(m, n, board, possibleValues)).start();
						Strategy temp = new SingleValueForCellStrategy(counter, m, n, board, possibleValues);
						startThread(temp, threadPool);
					} catch (Exception e) {
		...
		//System.out.println("Should equal number of empty spaces: " + counter);
	}
